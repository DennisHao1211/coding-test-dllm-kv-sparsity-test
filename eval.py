...
class Fast_dLLM_v2EvalHarness(LM):
    def __init__(
        ...
        **kwargs,
        ...
        self.model, self.tokenizer, _ = load_causal_lm_and_tokenizer(
            model_path,
            code_path=code_path,
        ...
        self.block_cache_keep_ratio = block_cache_keep_ratio
        self.block_cache_pool_kernel_size = block_cache_pool_kernel_size
    ...
    def generate_until(self, requests):
        ...
        should_report_speed = self.show_speed and self.rank == 0
        if should_report_speed and self.device.type == "cuda":
        ...
        for _, batch in enumerate(tqdm(batched_requests, desc="Generating...")):
            ...
            with torch.no_grad():
                ...
                batch_generation_start = time.perf_counter() if should_report_speed else None
                ...
                if self.accelerator is not None:
                    generated_ids = self.accelerator.unwrap_model(self.model).mdm_sample(
                        ...
                        block_cache_keep_ratio=self.block_cache_keep_ratio,
                        block_cache_pool_kernel_size=self.block_cache_pool_kernel_size,
                    )
                else:
                ...
                if should_report_speed and self.device.type == "cuda":
                    torch.cuda.synchronize(self.device)
                if should_report_speed:
                    generation_time += time.perf_counter() - batch_generation_start
            ...
            for batch_pos, (orig_idx, req) in enumerate(batch):
                ...
                # count token number
                if should_report_speed:
                    num_tokens += int((generated_ids[batch_pos][seq_len[batch_pos]:] != self.mask_id).sum().item())
                
                # put result in the correct original index position
                output[orig_idx] = generated_answer

        if should_report_speed:
            elapsed = max(generation_time, 1e-12)
            print(f"Total number of tokens generated: {num_tokens}")
            print(f"Total generation time: {elapsed} seconds")
