...
class Fast_dLLM_QwenAttention(nn.Module):
    ...
    def forward(
        ...
        # 在 attention 里新增了 record_block_queries 和 query_state_collector 参数
        record_block_queries: Optional[bool] = False,
        query_state_collector: Optional[list] = None,
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # 在 forward 时把每层的 query_states 存下来
        # Sparse-dLLM adaptation: capture each layer's Q_b during the
        # full-block build step so prefix filtering can score historical KV.
        if record_block_queries:
            if query_state_collector is None:
                raise ValueError("query_state_collector must be provided when record_block_queries=True")
            if len(query_state_collector) <= self.layer_idx:
                query_state_collector.extend([None] * (self.layer_idx + 1 - len(query_state_collector)))
            query_state_collector[self.layer_idx] = query_states.detach().clone()

        # Fast-dLLM 的 local block cache 保持 dense；reuse 时只原位覆盖
        # 当前 small block 的区间，而不是重新构建整个 block cache。
        ...
        # 这是 Sparse-dLLM 的 AR 适配：past_key_value 里只有历史 prefix KV；
        # 实际复用的上下文是 prefix cache + 当前 block 的 dense local KV。
    ...
    def forward(
        ...
        **kwargs
        ...
        hidden_states = self.self_attn(
            ...
            record_block_queries=record_block_queries,
...
class Fast_dLLM_QwenForCausalLM(Fast_dLLM_QwenPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs
        ...
        if delay_step < 0:
            raise ValueError(f"delay_step must be non-negative, got {delay_step!r}")
            ...
            while True:
                ...
                for small_block_idx in range(num_small_blocks):
                    ...
                    while True:
                        ...
                        # 根据 decode_step 和 delay_step 的关系，决定是否使用 block cache，以及如何使用 block cache
                        # < delay_step 走 dense/full-block; == delay_step build block cache; > delay_step reuse block cache
                        if use_block_cache:
